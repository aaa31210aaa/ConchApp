...
//    public static String host = "http://192.168.5.103:8080/CONCH-MES/mobile/";
    //匡
//    public static String host = "http://192.168.5.97:8080/CONCH-MES/mobile/";
    //飞
...
//     public static String host = "http://218.76.35.118:8884/CONCH-MES/mobile/";
    ...
    public static String getTeamId(Context context) {
        return SharedPrefsUtil.getValue(context, "userInfo", "teamid", "");
    }

    //获取用户授权台帐名称
    public static String getLedgerName(Context context) {
        String LedgerName = "";
        if (getTeamId(context).equals("10000301") || getTeamId(context).equals("10000302") || getTeamId(context).equals("10000303")) {
            LedgerName = "卡车和矿石转运";
        } else if (getTeamId(context).equals("10000307")) {
            LedgerName = "破碎台帐(生产)";
        } else if (getTeamId(context).equals("36ba2ed365c84b79a676932369dacc30")) {
            LedgerName = "破碎台帐(局控)";
        } else if (getTeamId(context).equals("10000308")) {
            LedgerName = "穿孔和爆破";
        } else if (getTeamId(context).equals("10000309") || getTeamId(context).equals("10000311")) {
            LedgerName = "铲装台帐";
        }
        return LedgerName;
    }
    ...
    //删除一条炮孔数据
    public static String DeleteBlasthole() {
        return host + "hole/deletePkInfo.action";
    }

    //获取炮孔分组信息详情
    public static String getBlastholeDetail() {
        return host + "hole/selectInfoByProjectid.action";
    }

    //分组信息上传
    public static String upLoadGroupInfo() {
        return host + "hole/saveFzInfo.action";
    }

    //保存分组信息
    public static String SaveFzInfo() {
        return host + "hole/holeAssayGroup.action";
    }
    ...
    //卡车台帐操作设备
    public static String TruckList() {
        return host + "pbdetail/selectDeviceList.action";
    }

    //卡车台帐设备列表
    public static String TruckDeviceList() {
        return host + "pbdetail/selectAllDevice.action";
    }

    //新增修改保存卡车台账设备
    public static String SaveTruckDeviceList() {
        return host + "pbdetail/doModifySave.action";
    }
}
